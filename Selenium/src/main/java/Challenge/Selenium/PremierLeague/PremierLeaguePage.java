...
public class PremierLeaguePage extends TestBase {
	public static final Logger log = Logger.getLogger(PremierLeaguePage.class.getName());
	...
	public void verifyPremierLeaguePage() {
		if (verifyPremierLeague.isDisplayed()) {
			...
			try {
				getScreenshot(driver, "PremierLeagueverificationFailed");
			} catch (IOException e) {
	...
	public void verifyArsenalOverviewPage() {
		if (verifyArsenalOverview.isDisplayed()) {
			...
			try {
				getScreenshot(driver, "ArsenalPageverificationFailed");
			} catch (IOException e) {
				log.info("Failed to get screenshot");
			}
		}
